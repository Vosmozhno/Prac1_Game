...
# Импорт библиотеки time для создания пауз между действиями
import time

# Функция для отображения информации о начале игры
def start_game():
...
# Функция для выбора действий на уровне
...
def main():
    inventory = []  # Инвентарь игрока (список)
    start_game()
    level_1(inventory)
    level_2(inventory)
    level_3(inventory)
